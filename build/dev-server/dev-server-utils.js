...
let resolveIds = 1;
export function sendMsg(prcs, msg) {
    prcs.send(msg);
}
export function sendMsgWithResponse(prcs, msg) {
...
export function createMessageReceiver(prcs, cb) {
    prcs.on('message', (msg) => {
...
export function sendError(prcs, e) {
    const msg = {
        error: {
            message: e,
        },
...
export function getBrowserUrl(protocol, address, port, basePath, pathname) {
...
export function getDevServerClientUrl(devServerConfig, host) {
...
export function getContentType(devServerConfig, filePath) {
...
export function isSimpleText(filePath) {
    const ext = filePath
        .toLowerCase()
        .trim()
        .split('.')
...
export function isDevClient(pathname) {
    return pathname.startsWith(c.DEV_SERVER_URL);
}
export function isDevModule(pathname) {
    return pathname.includes(c.DEV_MODULE_URL);
}
export function isOpenInEditor(pathname) {
    return pathname === c.OPEN_IN_EDITOR_URL;
}
export function isInitialDevServerLoad(pathname) {
    return pathname === c.DEV_SERVER_INIT_URL;
}
export function isDevServerClient(pathname) {
    return pathname === c.DEV_SERVER_URL;
}
export function shouldCompress(devServerConfig, req) {
