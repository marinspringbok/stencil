import { URL } from 'url';
export function normalizeHydrateOptions(inputOpts) {
...
export function generateHydrateResults(opts) {
...
export function renderBuildDiagnostic(results, level, header, msg) {
    const diagnostic = {
        level: level,
        type: 'build',
        header: header,
...
export function renderBuildError(results, msg) {
    return renderBuildDiagnostic(results, 'error', 'Hydrate Error', msg);
}
export function renderCatchError(results, err) {
