...
export function resolveVar(props, prop, fallback) {
    if (props[prop]) {
        return props[prop];
    }
    if (fallback) {
...
export function findVarEndIndex(cssText, offset) {
    let count = 0;
    let i = offset;
...
export function parseVar(cssText, offset) {
...
export function compileVar(cssText, template, offset) {
...
export function executeTemplate(template, props) {
    let final = '';
...
export function findEndValue(cssText, offset) {
    let onStr = false;
    let double = false;
    let i = offset;
    for (; i < cssText.length; i++) {
...
export function removeCustomAssigns(cssText) {
    let final = '';
    let offset = 0;
    while (true) {
...
export function compileTemplate(cssText) {
    let index = 0;
