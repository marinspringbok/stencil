...
export const parseCssImports = async (config, compilerCtx, buildCtx, srcFilePath, resolvedFilePath, styleText, styleDocs) => {
...
export const getCssImports = async (config, compilerCtx, buildCtx, filePath, styleText) => {
    const imports = [];
...
export const isCssNodeModule = (url) => url.startsWith('~');
export const resolveCssNodeModule = async (config, compilerCtx, diagnostics, filePath, cssImportData) => {
    try {
...
export const isLocalCssImport = (srcImport) => {
...
export const replaceNodeModuleUrl = (baseCssFilePath, moduleId, nodeModulePath, url) => {
...
export const replaceImportDeclarations = (styleText, cssImports, isCssEntry) => {
    for (const cssImport of cssImports) {
        if (isCssEntry) {
