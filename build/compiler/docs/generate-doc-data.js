...
export const generateDocData = async (config, compilerCtx, buildCtx) => {
    return {
...
const getDocsComponents = async (config, compilerCtx, buildCtx) => {
    const results = await Promise.all(buildCtx.moduleFiles.map(async (moduleFile) => {
        ...
        const readmePath = normalizePath(join(dirPath, 'readme.mdx'));
...
export const getNameText = (name, tags) => {
    return tags
